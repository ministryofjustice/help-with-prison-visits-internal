...
module.exports = router => {
  router.post('/audit/print-report', (req, res, next) => {
    authorisation.hasRoles(req, [applicationRoles.BAND_9, applicationRoles.CASEWORK_MANAGER_BAND_5])
    const reportId = req.body?.reportId

    getReportData(reportId).then(claims => {
      getReportDates(reportId).then(dates => {
        if (claims) {
          const checkedClaimCount = claims.filter(
            claim =>
              claim.Band5Validity === audit.CLAIM_STATUS.VALID || claim.Band5Validity === audit.CLAIM_STATUS.INVALID,
          ).length
          const validCheckedClaimCount = claims.filter(claim => claim.Band5Validity === audit.CLAIM_STATUS.VALID).length
          const invalidCheckedClaimCount = claims.filter(
            claim => claim.Band5Validity === audit.CLAIM_STATUS.INVALID,
          ).length
          const claimSelectedForVerificationCount = claims.filter(
            claim => claim.Band9Validity !== audit.STATUS.NOT_REQUIRED,
          ).length
          const verifiedClaimCount = claims.filter(
            claim =>
              claim.Band9Validity === audit.CLAIM_STATUS.VALID || claim.Band9Validity === audit.CLAIM_STATUS.INVALID,
          ).length
          const validVerifiedClaimCount = claims.filter(
            claim => claim.Band9Validity === audit.CLAIM_STATUS.VALID,
          ).length
          ...
          const invalidConfirmedClaimCount = claims.filter(
            claim =>
              claim.Band5Validity === audit.CLAIM_STATUS.INVALID && claim.Band9Validity === audit.CLAIM_STATUS.INVALID,
          ).length
          const validConfirmedClaimCount = claims.filter(
            claim =>
              claim.Band5Validity === audit.CLAIM_STATUS.INVALID && claim.Band9Validity === audit.CLAIM_STATUS.VALID,
          ).length

          const totalCheckedAmount = claims.reduce((n, { PaymentAmount }) => n + PaymentAmount, 0)
