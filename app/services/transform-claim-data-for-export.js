...
const getClaimEscorts = require('./data/get-claim-escorts')
const getClaimChildCounts = require('./data/get-claim-child-counts')
const getClaimExpensesForClaims = require('./data/get-claim-expenses-for-claims')
...
module.exports = claims => {
  ...
  return Promise.all([getClaimChildCounts(claimIds), getClaimEscorts(claimIds), getClaimExpensesForClaims(claimIds)])
    .then(data => {
      return Promise.map(claims, claim => {
        const returnValue = {}

        const childCount = data[0].find(childCountRow => {
          return childCountRow.ClaimId === claim.ClaimId
        })
        const hasEscort =
          data[1].find(escortRow => {
            return escortRow.ClaimId === claim.ClaimId
          }) !== undefined
        const claimExpenses = data[2].filter(expenseRow => {
          return expenseRow.ClaimId === claim.ClaimId
        })
        ...
        returnValue[DATE_REVIEWED_BY_CASEWORKER_HEADER] = claim.DateReviewed
          ? dateHelper.shortDate(claim.DateReviewed)
          : null
        returnValue[IS_ADVANCE_CLAIM_HEADER] = claim.IsAdvanceClaim ? 'Y' : 'N'
        returnValue[TOTAL_AMOUNT_PAID_HEADER] = totalAmountPaid
        returnValue[PAYMENT_METHOD_HEADER] = claim.PaymentMethod
          ? displayHelper.getPaymentMethodDisplayName(claim.PaymentMethod)
          : ''
