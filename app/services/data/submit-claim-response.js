...
module.exports = (claimId, claimDecision) => {
  ...
  return db('Claim')
    ...
    .then(result => {
      ...
      const { caseworker } = claimDecision
      const { decision } = claimDecision
      const { note } = claimDecision
      const { nomisCheck } = claimDecision
      const { releaseDateIsSet } = claimDecision
      let releaseDate = null
      if (claimDecision.releaseDateIsSet) {
        releaseDate = claimDecision.releaseDate.format('YYYY-MM-DD')
      }
      const { dwpCheck } = claimDecision
      const { visitConfirmationCheck } = claimDecision
      const { rejectionReasonId } = claimDecision
      ...
      return Promise.all([
        updateEligibility(eligibilityId, decision),
        updateClaim(
          claimId,
          caseworker,
          decision,
...
function updateClaim(
  ...
  return db('Claim')
    .where('ClaimId', claimId)
    .update(updateObject)
    .then(log.info(`Claim ID ${claimId} Closed with Status: ${updateObject.Status}`))
}
...
function updateClaimExpenses(claimExpenseResponses) {
  ...
  if (claimExpenseResponses) {
    claimExpenseResponses.forEach(claimExpenseResponse => {
