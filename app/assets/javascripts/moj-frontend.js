;(function (root, factory) {
  ...
  MOJFrontend.FormValidator.prototype.showInlineError = function (error) {
    ...
    const errorSpan = '<span class="govuk-error-message" id="' + errorSpanId + '">' + this.escapeHtml(error.message) + '</span>'
  ...
  if (MOJFrontend.dragAndDropSupported() && MOJFrontend.formDataSupported() && MOJFrontend.fileApiSupported()) {
    ...
    MOJFrontend.MultiFileUpload.prototype.getDeleteButtonHtml = function (file) {
	  let html = '<button class="moj-multi-file-upload__delete govuk-button govuk-button--secondary govuk-!-margin-bottom-0" type="button" name="delete" value="' + file.filename + '">'
