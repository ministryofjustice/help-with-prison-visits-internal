...
describe('services/data/dashboard/get-auto-approved-claim-count', () => {
  describe('module', () => {
    beforeAll(() => {
    ...
    it('should return the correct number of Auto-Approved claims submitted today', () => {
      const expectedResult = todayCount + 1
      return checkCount(dashboardFilterEnum.TODAY, expectedResult)
    })

    it('should return the correct number of Auto-Approved claims submitted yesterday', () => {
      const expectedResult = yesterdayCount + 1
      return checkCount(dashboardFilterEnum.YESTERDAY, expectedResult)
    })

    it('should return the correct number of Auto-Approved claims submitted in the last 7 days', () => {
      const expectedResult = last7DaysCount + 3
      return checkCount(dashboardFilterEnum.LAST_7_DAYS, expectedResult)
    })

    it('should return the correct number of Auto-Approved claims submitted in the previous calendar month', () => {
    ...
    it('should return the correct number of Auto-Approved claims submitted in the calendar month before the previous month', () => {
      const expectedResult = twoMonthsAgoCount + 1
      return checkCount(dashboardFilterEnum.TWO_MONTHS_AGO, expectedResult)
    })

    it('should return the correct number of Auto-Approved claims submitted in the calendar month 3 months ago', () => {
      const expectedResult = threeMonthsAgoCount + 1
      return checkCount(dashboardFilterEnum.THREE_MONTHS_AGO, expectedResult)
    })

    it('should return the correct number of Auto-Approved claims submitted in the calendar month 4 months ago', () => {
...
function getCountsBeforeTest () {
  ...
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.TODAY).then(result => { todayCount = result[0].Count }))
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.YESTERDAY).then(result => { yesterdayCount = result[0].Count }))
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.LAST_7_DAYS).then(result => { last7DaysCount = result[0].Count }))
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.ONE_MONTH_AGO).then(result => { oneMonthAgoCount = result[0].Count }))
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.TWO_MONTHS_AGO).then(result => { twoMonthsAgoCount = result[0].Count }))
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.THREE_MONTHS_AGO).then(result => { threeMonthsAgoCount = result[0].Count }))
  promises.push(getAutoApprovedClaimCount(dashboardFilterEnum.FOUR_MONTHS_AGO).then(result => { fourMonthsAgoCount = result[0].Count }))
