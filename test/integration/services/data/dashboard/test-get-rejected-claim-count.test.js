...
describe('services/data/dashboard/get-rejected-claim-count', () => {
  describe('module', () => {
    beforeAll(() => {
      return getCountsBeforeTest()
        ...
        .then(function (ids) {
          ...
          promises.push(insertClaim(claimId4, eligibilityId, reference, oneMonthAgo, claimStatusEnum.REJECTED.value, false))
          promises.push(insertClaim(claimId5, eligibilityId, reference, twoMonthsAgo, claimStatusEnum.REJECTED.value, false))
          promises.push(insertClaim(claimId6, eligibilityId, reference, threeMonthsAgo, claimStatusEnum.REJECTED.value, false))
          promises.push(insertClaim(claimId7, eligibilityId, reference, fourMonthsAgo, claimStatusEnum.REJECTED.value, false))
          promises.push(insertClaim(claimId8, eligibilityId, reference, yesterday, claimStatusEnum.APPROVED.value, false))
          promises.push(insertClaim(claimId9, eligibilityId, reference, oneMonthAgo, claimStatusEnum.PENDING.value, false))
...
function getCountsBeforeTest () {
  ...
  promises.push(getRejectedClaimCount(dashboardFilterEnum.TODAY).then(result => { todayCount = result[0].Count }))
  promises.push(getRejectedClaimCount(dashboardFilterEnum.YESTERDAY).then(result => { yesterdayCount = result[0].Count }))
  promises.push(getRejectedClaimCount(dashboardFilterEnum.LAST_7_DAYS).then(result => { last7DaysCount = result[0].Count }))
  promises.push(getRejectedClaimCount(dashboardFilterEnum.ONE_MONTH_AGO).then(result => { oneMonthAgoCount = result[0].Count }))
  promises.push(getRejectedClaimCount(dashboardFilterEnum.TWO_MONTHS_AGO).then(result => { twoMonthsAgoCount = result[0].Count }))
  promises.push(getRejectedClaimCount(dashboardFilterEnum.THREE_MONTHS_AGO).then(result => { threeMonthsAgoCount = result[0].Count }))
  promises.push(getRejectedClaimCount(dashboardFilterEnum.FOUR_MONTHS_AGO).then(result => { fourMonthsAgoCount = result[0].Count }))
