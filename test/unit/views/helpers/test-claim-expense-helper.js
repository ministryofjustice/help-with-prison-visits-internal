...
describe('views/helpers/claim-expense-helper', function () {
  describe('FormattedDetail', function () {
    ...
    it('should use prison display name for car, toll and parking expenses', function () {
      expect(claimExpenseHelper({ ExpenseType: 'car', From: FROM, To: TO_PRISON })).toBe(`${FROM} to ${TO_PRISON_DISPLAY_NAME}`)

      expect(claimExpenseHelper({ ExpenseType: 'toll', From: FROM, To: TO_PRISON })).toBe(`${FROM} to ${TO_PRISON_DISPLAY_NAME}`)

      expect(claimExpenseHelper({ ExpenseType: 'parking', From: FROM, To: TO_PRISON })).toBe(`${FROM} to ${TO_PRISON_DISPLAY_NAME}`)
    })
