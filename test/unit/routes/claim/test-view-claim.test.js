...
describe('routes/claim/view-claim', () => {
  let app
  ...
  beforeEach(() => {
    ...
    jest.mock('../../../../app/routes/helpers/get-claim-expense-responses', () => mockGetClaimExpenseResponses)
    jest.mock('../../../../app/services/data/get-claim-last-updated', () => mockGetClaimLastUpdated)
    jest.mock('../../../../app/services/check-user-and-last-updated', () => mockCheckUserAndLastUpdated)
    ...
    jest.mock('../../../../app/services/data/update-claim-overpayment-status', () => mockUpdateClaimOverpaymentStatus)
    jest.mock('../../../../app/services/domain/overpayment-response', () => mockOverpaymentResponse)
    jest.mock('../../../../app/services/data/close-advance-claim', () => mockCloseAdvanceClaim)
    jest.mock('../../../../app/services/data/payout-barcode-expired-claim', () => mockPayoutBarcodeExpiredClaim)
    ...
    jest.mock('../../../../app/services/data/request-new-bank-details', () => mockRequestNewBankDetails)
    ...
    jest.mock('../../../../app/services/data/update-assignment-of-claims', () => mockUpdateAssignmentOfClaims)
  ...
  describe('POST /claim/:claimId', () => {
    it('should respond with 302 when valid data entered', () => {
    ...
    it('should respond with a 500 for an unhandled exception', () => {
      mockCheckUserAndLastUpdated.mockResolvedValue()
      mockClaimDecision.mockReturnValue()
      mockGetClaimExpenseResponses.mockReturnValue()
      mockSubmitClaimResponse.mockRejectedValue()

      return supertest(app).post('/claim/123').send(VALID_DATA_APPROVE).expect(500)
    })

    it('should call updateEligibilityTrustedStatus if claim decision is APPROVED', () => {
    ...
    it('should respond with 400 when user and user and last updated check throws validation error', () => {
      mockCheckUserAndLastUpdated.mockImplementation(() => {
    ...
    it('should respond with 400 when invalid data entered', () => {
      mockClaimDecision.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
      })
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockResolvedValue()
    ...
    it('should respond with a 500 when promise is rejected', () => {
      mockCheckUserAndLastUpdated.mockResolvedValue()
      mockClaimDecision.mockReturnValue({})
    ...
    it('should populate NomisCheck, DWPCheck and VisitConfirmationCheck when handling error and data is available', () => {
      const claimDetails = {
        claim: {},
        claimExpenses: [{}],
      }
      ...
      mockCheckUserAndLastUpdated.mockResolvedValue()
      mockGetClaimExpenseResponses.mockReturnValue([{}])
      mockSubmitClaimResponse.mockImplementation(() => {
  ...
  describe('GET /claim/:claimId/download', () => {
    it('should respond with 200 if a valid file path is returned', () => {
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockGetClaimDocumentFilePath.mockResolvedValue(CLAIM_DOCUMENT)
      return supertest(app).get('/claim/123/download?claim-document-id=55').expect(200).expect('content-length', '4')
    })

    it('should respond with 500 if no claim-document-id provided', () => {
      return supertest(app).get('/claim/123/download').expect(500)
    })

    it('should respond with 500 if no file path is provided', () => {
      mockGetClaimDocumentFilePath.mockResolvedValue()
      return supertest(app).get('/claim/123/download?claim-document-id=55').expect(500)
    })
  })

  describe('POST /claim/:claimId/add-deduction', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
      })
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
    ...
    it('should respond with 400 including extra data if claim expenses exist and there is no update conflict when a validation error occurs', () => {
      const claimData = {
        claim: {},
        claimExpenses: {},
      }
      ...
      mockClaimDecision.mockReturnValue({})
      mockInsertDeduction.mockImplementation(() => {
        throw new ValidationError()
      })
      mockMergeClaimExpensesWithSubmittedResponses.mockReturnValue({})
    ...
    it('should respond with 302 when valid data entered (add deduction)', () => {
      const claimData = {
        claim: {},
        claimExpenses: {},
      }
    ...
    it('should respond with a 500 when promise is rejected', () => {
      ...
      return supertest(app).post('/claim/123/add-deduction').send(VALID_DATA_ADD_DEDUCTION).expect(500)
    })
  })

  describe('POST /claim/:claimId/remove-deduction', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
      })
    ...
    it('should respond with 302 when valid data entered (disable deduction)', () => {
      const claimData = {
        claim: {},
        claimExpenses: {},
      }
    ...
    it('should respond with a 500 when promise is rejected', () => {
      ...
      return supertest(app).post('/claim/123/remove-deduction').send(VALID_DATA_DISABLE_DEDUCTION).expect(500)
    })
  })

  describe('POST /claim/:claimId/assign-self', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
      })
    ...
    it('should respond with 200 after calling assignment with users email if no conflicts', () => {
      const claimData = {
        claim: {},
        claimExpenses: {},
      }
    ...
    it('should respond with a 500 when promise is rejected', () => {
      ...
      return supertest(app).post('/claim/123/assign-self').send().expect(500)
    })
  })

  describe('POST /claim/:claimId/unassign', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
      })
    ...
    it('should respond with a 500 when promise is rejected', () => {
      ...
      return supertest(app).post('/claim/123/unassign').send().expect(500)
    })
  })

  describe('POST /claim/:claimId/update-overpayment-status', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
    ...
    it('should respond with 400 when adding an overpayment and a validation error occurs', () => {
      const claimData = { claim: { IsOverpaid: false } }
      const overpaymentResponse = {}
      mockGetIndividualClaimDetails.mockResolvedValue(claimData)
      mockOverpaymentResponse.mockReturnValue(overpaymentResponse)
      mockCheckUserAndLastUpdated.mockResolvedValue()
      mockUpdateClaimOverpaymentStatus.mockImplementation(() => {
        throw new ValidationError()
      })
    ...
    it('should respond with a 500 when promise is rejected', () => {
      ...
      return supertest(app).post('/claim/123/update-overpayment-status').send(VALID_DATA_DISABLE_DEDUCTION).expect(500)
    })
  })

  describe('POST /claim/:claimId/payout-barcode-expired', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
    ...
    it('should respond with 400 when marking claim as payout expired and a validation error occurs', () => {
      mockPayoutBarcodeExpiredClaim.mockImplementation(() => {
        throw new ValidationError()
      })
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockResolvedValue()
  ...
  describe('POST /claim/:claimId/insert-note', () => {
    it('should respond with 302 when valid data entered', () => {
    ...
    it('should respond with a 500 when no field blank', () => {
      mockInsertNote.mockResolvedValue()
      mockCheckUserAndLastUpdated.mockResolvedValue()

      return supertest(app).post('/claim/123/insert-note').send(INVALID_DATA_INSERT_NOTE).expect(500)
    })
  })

  describe('POST /claim/:claimId/close-advance-claim', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
    ...
    it('should respond with 400 when closing claim and a validation error occurs', () => {
      mockCloseAdvanceClaim.mockImplementation(() => {
        throw new ValidationError()
      })
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockResolvedValue()
    ...
    it('should respond with a 500 when promise is rejected', () => {
      ...
      return supertest(app).post('/claim/123/close-advance-claim').send(VALID_DATA_DISABLE_DEDUCTION).expect(500)
    })
  })

  describe('POST /claim/:claimId/request-new-payment-details', () => {
    it('should respond with 400 when user and last updated check throws validation error', () => {
      mockGetIndividualClaimDetails.mockResolvedValue(CLAIM_RETURN)
      mockCheckUserAndLastUpdated.mockImplementation(() => {
        throw new ValidationError({ reason: {} })
    ...
    it('should respond with 302 when request bank details submitted', () => {
  ...
  describe('POST /claim/:claimId/update-benefit-expiry-date', () => {
    it('should respond with 302 when valid benefit expiry date is entered', () => {
      const claimData = {
        claim: {},
        claimExpenses: {},
      }
      ...
      const benefitExpiryDate = {
        expiryDateFields: [
          VALID_BENEFIT_EXPIRY_DATA['expiry-day-input'],
          VALID_BENEFIT_EXPIRY_DATA['expiry-month-input'],
          VALID_BENEFIT_EXPIRY_DATA['expiry-year-input'],
        ],
        expiryDate: dateFormatter.build(
          VALID_BENEFIT_EXPIRY_DATA['expiry-day-input'],
          VALID_BENEFIT_EXPIRY_DATA['expiry-month-input'],
          VALID_BENEFIT_EXPIRY_DATA['expiry-year-input'],
        ),
  ...
  describe('POST /claim/:claimId/add-top-up', () => {
    it('should respond with 302 when valid top up data entered', () => {
      mockCheckUserAndLastUpdated.mockResolvedValue()
      ...
      return supertest(app)
        ...
        .expect(() => {
          ...
          expect(mockInsertTopUp).toHaveBeenCalledWith(
            { Reference: 'TOPUP', EligibilityId: '1', PaymentStatus: 'PROCESSED' },
            topUpResponse,
            'test@test.com',
          )
        })
  ...
  describe('POST /claim/:claimId/cancel-top-up', () => {
    it('should respond with 302 when valid cancel top up data is sent', () => {
      mockCheckUserAndLastUpdated.mockResolvedValue()
      mockCancelTopUp.mockResolvedValue()
      mockGetIndividualClaimDetails.mockResolvedValue({
        claim: {
      ...
      return supertest(app)
        ...
        .expect(() => {
          ...
          expect(mockCancelTopUp).toHaveBeenCalledWith(
            { Reference: 'CANCEL', EligibilityId: '1', PaymentStatus: 'PROCESSED' },
            'test@test.com',
          )
        })
    })
