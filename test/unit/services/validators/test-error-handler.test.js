...
describe('services/validators/error-handler', () => {
  ...
  describe('add', () => {
    it('should return single result if one error is added', done => {
    ...
    it('should have three errors saved against the field name if add is called three times', done => {
      errorHandler.add(FIELD_NAME_1, message, OPTIONS)
      errorHandler.add(FIELD_NAME_1, message, OPTIONS)
      errorHandler.add(FIELD_NAME_1, message, OPTIONS)
      const result = errorHandler.get()
    ...
    it('should have a property and error for each differnet field name passed to add', done => {
      errorHandler.add(FIELD_NAME_1, message, OPTIONS)
  ...
  describe('get', () => {
    it('should return false if no errors are added', done => {
      const result = errorHandler.get()
