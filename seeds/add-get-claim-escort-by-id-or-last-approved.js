  ...
  return knex.schema
    ...
    .then(function () {
      return knex.schema
        .raw(
          `
            CREATE FUNCTION IntSchema.getClaimEscortByIdOrLastApproved(@reference varchar(7), @eligibiltyId int, @claimId int)
            RETURNS TABLE
            AS
            RETURN
            (
        ...
        .raw('GRANT SELECT ON IntSchema.getClaimEscortByIdOrLastApproved TO ??;', [config.EXT_WEB_USERNAME])
    })
